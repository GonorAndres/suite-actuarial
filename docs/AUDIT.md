...
- **Mortalidad sintética.** `data/mortality_tables/emssa_09.csv` es una rampa lineal
...
| --- | --- | --- | --- | --- | --- |
| Tabla de mortalidad `emssa_09.csv` | Ninguna: construida para el laboratorio | No aplica | Rampa lineal de qx, no Gompertz; q₆₅ ≈ 0.0135 contra ≈0.02 de la EMSSAH real | `data_status: illustrative` en metadatos; `TablaMortalidad.__repr__` | Licenciar la EMSSA-09 publicada y cargarla con su hash; la interfaz ya la admite |
...
| GMM y accidentes | Docstring del módulo | No aplica | Sin frecuencia-severidad ni tendencia médica; `siniestralidad = prima/(1+margen)` es circular | `ExperimentalModelWarning` de salud | Modelar frecuencia y severidad con datos de experiencia |
...
| Tarifas de auto | Tablas AMIS ilustrativas | No aplica | Redondeo tras cada factor; la RC de terceros se tarifica sobre el valor del propio vehículo | Esta auditoría | Redondear solo al final; tarificar RC sobre límite de responsabilidad |
...
| Cita LISR Art. 151 fracc. V (planes de retiro) | `validador_primas.py` | Sin verificar | Material del SAT etiqueta esta deducción como fracc. VII; además el código calcula el tope al 15% mientras su propio comentario dice 10% | Esta auditoría (pendiente Clase A de deducibilidad) | Leer el Art. 151 vigente, corregir tasa y fracción con un test derivado del estatuto |
| Tope global de deducciones personales (GMM, persona física) | No implementado | Sin verificar | `gastos_medicos` devuelve 100% deducible sin aplicar el tope del último párrafo del Art. 151 | Esta auditoría | Aplicar `min(5 UMA, 15% del ingreso)` tras verificar el texto; requiere `ingreso_anual`, que el API ya acepta |
