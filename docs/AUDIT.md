...
| Escala de Bonus-Malus (`danos/tarifas.py`) | Ninguna: los nueve niveles (−5 a 3), sus factores (0.70 a 1.50) y las reglas de transición se construyeron para el laboratorio. El módulo los llamaba «escala estándar mexicana» sin fuente (corregido 2026-08-02) | No aplica | La escala no está calibrada: nada garantiza que descuentos y recargos se compensen sobre una cartera, ni que la distancia entre niveles corresponda a una diferencia de riesgo medida. `tests/unit/test_danos_tarifas.py::TestAvisoBonusMalus` lo hace visible: con un siniestro cada dos periodos el asegurado deriva al recargo máximo y se queda ahí | `ExperimentalModelWarning` al construir `CalculadoraBonusMalus` (2026-08-02); `DISCLAIMER` del módulo; `disclaimer` y `validation_tier` en la respuesta del API | Sustituir niveles, factores y reglas por los de la escala registrada de la institución, y calibrarlos contra su experiencia |
| Modelo colectivo (`danos/frecuencia_severidad.py`) | El método es estándar (Bowers et al.); los parámetros los fija quien llama y no se ajustan a dato alguno | No aplica | No estima parámetros: los recibe. Supone N y X independientes y las X idénticamente distribuidas; no reconoce deducible, límite por siniestro, reaseguro, inflación ni descuento; trata los parámetros como conocidos, así que VaR y TVaR no incorporan incertidumbre de parámetro y su error de muestreo Monte Carlo no se reporta (al 99% la cifra descansa sobre el 1% de las simulaciones) | `ExperimentalModelWarning` al construir `ModeloColectivo` (2026-08-02); `DISCLAIMER` del módulo; `disclaimer` y `validation_tier` en la respuesta del API; leyenda equivalente en la pestaña de Streamlit | Ajustar las distribuciones a experiencia propia y reportar el error de estimación de las medidas de cola |
