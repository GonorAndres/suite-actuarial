...
export const DOMAIN_GUIDES: Record<DomainId, DomainGuideDefinition> = {
  vida: {
    ...
    interpretation: text("La sensibilidad separa el efecto de la exposición, que son la suma asegurada y el plazo, del efecto biométrico de la edad.", "Sensitivity separates the exposure effect, meaning amount and term, from the biometric effect of age."),
  ...
  salud: {
    ...
    decision: text("Ver juntos la prima, el deducible, el coaseguro, el tope y la suma asegurada.", "Look at premium, deductible, coinsurance, cap, and sum insured together."),
    ...
    limitations: [text("No hay un modelo de frecuencia-severidad alimentado con datos de experiencia.", "There is no frequency-severity model fed by experience data."), text("La siniestralidad sale de la propia prima, así que es circular y sólo ilustrativa.", "The loss ratio comes out of the premium itself, so it is circular and illustrative only.")],
    workbenchModel: "gmm",
  },
  pensiones: {
    id: "pensiones",
    ...
    decision: text("Distinguir entre un beneficio que fijan las reglas y una renta que financia el saldo individual.", "Distinguish a benefit set by rules from an annuity funded by an individual balance."),
  ...
  reservas: {
    ...
    validation: [text("Mack reproduce el caso de Taylor y Ashe hasta el peso.", "Mack reproduces the Taylor and Ashe case to the currency unit."), text("El bootstrap ODP reproduce el φ publicado y concilia con Chain Ladder.", "The ODP bootstrap reproduces the published φ and reconciles with Chain Ladder.")],
  ...
  reaseguro: {
    ...
    assumptions: [
      ...
      { name: text("Prima de reinstalación", "Reinstatement premium"), value: text("Prorrateo por monto", "Pro rata as to amount"), source: text("Simplificación declarada", "Declared simplification"), status: "method" },
    ],
    ...
    limitations: [text("El prorrateo de reinstalación no considera tiempo ni tasas escalonadas.", "Reinstatement proration considers neither time nor tiered rates."), text("Lo que se entiende por resultado neto cambia de un contrato a otro; la diferencia queda registrada en el inventario.", "What counts as net result changes from treaty to treaty; the difference is recorded in the inventory." )],
    workbenchModel: "xl",
  },
  regulatorio: {
    id: "regulatorio",
    ...
    assumptions: [
      { name: text("Factores RCS", "RCS factors"), value: text("Porcentajes fijos y escalones por tramo", "Fixed percentages and stepped bands"), source: text("Heurísticas pedagógicas, no CNSF", "Pedagogical heuristics, not CNSF"), status: "illustrative" },
    ...
    validation: [text("El índice capital/RCS se prueba por escala, por frontera y por insuficiencia.", "The capital/RCS ratio is tested by scale, by boundary, and by insufficiency."), text("Identidad: con la matriz usada, el agregado nunca excede la suma de los módulos.", "Identity: under the matrix used, the aggregate never exceeds the sum of the modules." )],
    limitations: [text("Los factores no implementan el modelo estocástico completo de la CNSF.", "Factors do not implement the complete CNSF stochastic model."), text("Las rutas del SAT traen tasas y citas todavía sin verificar, por eso se muestran como indeterminadas.", "SAT paths carry rates and citations that are still unverified, which is why they are shown as indeterminate." )],
    workbenchModel: "rcs",
  },
};
